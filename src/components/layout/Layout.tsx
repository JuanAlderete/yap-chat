...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return (
    <SidebarProvider>
      <Sidebar />
      <main className="flex flex-1 flex-col relative h-[100dvh] overflow-hidden">
        {(!isMobile || !hasActiveChat) && (
          <div className={`flex items-center gap-2 md:gap-4 absolute top-2 z-10 ${
            ...
            <SidebarTrigger className="bg-sidebar hover:bg-sidebar-accent hover:text-sidebar-accent-foreground flex-shrink-0 h-8 w-8 md:h-9 md:w-9" />
